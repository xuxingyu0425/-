...
def KNN_arithm(iris_datasets, expected_data):  # 定义knn算法函数，函数参数为训练集和测试集
    ...
    for j in range( 0, len( expected_data ) ):
        ...
        iris1 = []  # 用来存放测试集数据与第一类鸢尾花数据的闵可夫斯基距离
        iris2 = []  # 用来存放测试集数据与第二类鸢尾花数据的闵可夫斯基距离
        iris3 = []  # 用来存放测试集数据与第三类鸢尾花数据的闵可夫斯基距离
        index1 = 0  # 测试集数据与第一类鸢尾花邻近个数
        index2 = 0  # 测试集数据与第二类鸢尾花邻近个数
        index3 = 0  # 测试集数据与第三类鸢尾花邻近个数
        for i in range( 0, len( iris_data ) ):  # 计算闵可夫斯基距离
            distance.append(nth_root(pow(abs(expected_data[j][0]- iris_data[i][0]),0.5),3)+  nth_root(pow(abs(expected_data[j][1]- iris_data[i][1]),0.5),3)
                             + nth_root(pow(abs(expected_data[j][2]- iris_data[i][2]),0.5),3)+  nth_root(pow(abs(expected_data[j][3]- iris_data[i][3]),0.5),3))
            if (iris_target[i] == 0):
                iris1.append( nth_root(pow(abs(expected_data[j][0]- iris_data[i][0]),0.5),3)+  nth_root(pow(abs(expected_data[j][1]- iris_data[i][1]),0.5),3)
                             + nth_root(pow(abs(expected_data[j][2]- iris_data[i][2]),0.5),3)+  nth_root(pow(abs(expected_data[j][3]- iris_data[i][3]),0.5),3))
            elif (iris_target[i] == 1):
                iris2.append( nth_root(pow(abs(expected_data[j][0]- iris_data[i][0]),0.5),3)+  nth_root(pow(abs(expected_data[j][1]- iris_data[i][1]),0.5),3)
                             + nth_root( pow(abs(expected_data[j][2]- iris_data[i][2]),0.5),3)+ nth_root(pow(abs(expected_data[j][3]- iris_data[i][3]),0.5),3))
            elif (iris_target[i] == 2):
                iris3.append(  nth_root(pow(abs(expected_data[j][0]- iris_data[i][0]),0.5),3)+  nth_root(pow(abs(expected_data[j][1]- iris_data[i][1]),0.5),3)
                             + nth_root(pow(abs(expected_data[j][2]- iris_data[i][2]),0.5),3)+  nth_root(pow(abs(expected_data[j][3]- iris_data[i][3]),0.5),3))
